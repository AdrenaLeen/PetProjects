...
// Вызывает метод ToString() на каждом анонимном объекте.
foreach (object o in objs) Console.WriteLine(o);
Console.WriteLine();

foreach (ProductInfoSmall item in GetNamesAndDescriptionsTyped(itemsInStock)) Console.WriteLine(item.ToString());
Console.WriteLine();
